# https://github.com/wadecym/cnyes_twstock_news
import requests
import time
import json
...
def savefile(beginday,stopday,news):
    ...
    if not os.path.isfile(filename):
        with open(filename, 'a', newline='',encoding='utf8') as csvfile:
            ...
            writer.writerow(['date','time','title','tag','content','url'])
    with open(filename, 'a', newline='',encoding='utf8') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
...
def crawler(beginday,stopday):
    ...
    #獲取第一頁各個新聞的 newsId
    for index, i in enumerate(newsIDlist):
        # 如果title一樣就不要重複抓
        if index != 0:
            if i['title'] == newsIDlist[index-1]['title']:
                continue
    ...
    for p in range(2,last_page+1):
        ...
        try:
            ...
            continue
        for index, j in enumerate(newsIDlist):
            # 如果title一樣就不要重複抓
            if index != 0:
                if j['title'] == newsIDlist[index-1]['title']:
                    continue
    ...
    if os.path.isfile(filename):
        with open(filename, 'r', newline='',encoding='utf8') as csvfile:
            ...
            for row in reader:
                if row[0] == 'date':
                    continue
                id = int(row[5].split('/')[-1])
                if id in newsID_lt:
                    already += 1
                    newsID_lt.remove(id)
                else:
                    continue
            print('已經爬取過 {} 篇新聞'.format(already))
            print('剩下 {} 篇新聞'.format(len(newsID_lt)))
            csvfile.close()
    if(len(newsID_lt)==0):
      print('無新聞資料: {} ~ {}'.format(beginday,stopday))
      return
...
if __name__ == '__main__':
    beginyear=2013  #爬取新聞年份
    beginmonth=6    #爬取新聞開始月份
    stopmonth=12   #爬取停止月份
    startRange = 2 # 1:上半月開始 2:只爬下半月
    
    #爬取月份區間,因為測試直接抓一整個月伺服器回傳的資訊可能會出現異常,
    #所以分成上半月(1~15日)和下半月(16~月底)
    for i in range(startRange,3):    #先爬上半月再爬下半月
